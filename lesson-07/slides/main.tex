...
\begin{itemize}
    ...
    \item test healthcheck, DMZ Swagger access, login, CRUD endpoints, and read-only clinical data;
...
\midrule
Clinical & patients, admissions, admission types, lab APIs & clinical care needs broad access \\
Lab & laboratories, exams, exam types, exam rows & laboratory workflow, not full patient browsing \\
...
allowed calls, denied subnet calls, successful CRUD, and verified reads against real demo data.
\end{exampleblock}
\end{frame}

\begin{frame}[c]{What Is Actually CRUD?}
\begin{block}{Verified write workflows}
The guided lab performs real create/read/update/delete work only where the OpenHospital API accepts
the complete object model without extra clinical workflow setup.
\end{block}

\begin{exampleblock}{Hospital lens}
St. Isidore changes low-risk training records: admission-type configuration, lab exam-type
configuration, and a temporary patient created only for the exercise.
\end{exampleblock}
\end{frame}

\begin{frame}[fragile,c]{Administrative CRUD Requests}
\small
...
\begin{frame}[fragile,c]{Lab Configuration CRUD Requests}
\small
\begin{block}{API operations}
\begin{verbatim}
GET  /examtypes
...
\begin{exampleblock}{Network-security meaning}
The lab subnet can maintain laboratory exam-type configuration, but it still cannot access general
clinical administration endpoints such as \texttt{/admissiontypes}.
\end{exampleblock}
\end{frame}

\begin{frame}[fragile,c]{Patient CRUD Requests}
\small
\begin{block}{API operations}
\begin{verbatim}
POST /patients
GET  /patients/{created-code}
PUT  /patients/{created-code}
GET  /patients/{created-code}
DELETE /patients/{created-code}
\end{verbatim}
\end{block}

\begin{exampleblock}{Hospital lens}
The script creates a temporary training patient named \texttt{Security Patient}, updates it, checks
the result, and deletes it.
\end{exampleblock}
\end{frame}

\begin{frame}[fragile,c]{Verified Read-Only API Areas}
\small
\begin{block}{Real demo-data reads}
\begin{verbatim}
GET /exams/description/WBC
GET /examrows/byExamCode/01.02
GET /admissions?...page=0&size=3
GET /laboratories?oneWeek=false&page=0&size=3
GET /laboratories/15
\end{verbatim}
\end{block}

\begin{exampleblock}{Why not fake writes here?}
OpenHospital exams and laboratory requests are tied to existing clinical workflows and database
constraints. The lab therefore reads real records instead of inventing invalid objects.
\end{exampleblock}
\end{frame}
...
\takeawaysframe{
\begin{itemize}
    \item CRUD tests confirm useful API behavior where the model is simple enough for the lab.
    \item Complex clinical objects are tested through verified reads against real demo data.
...
\begin{exampleblock}{Hospital example}
The lab can reach laboratory API paths such as \texttt{/laboratories}, and the tutorial verifies this
with real read requests. It should not browse all \texttt{/patients} endpoints just because it is
inside the hospital.
\end{exampleblock}
\end{frame}
